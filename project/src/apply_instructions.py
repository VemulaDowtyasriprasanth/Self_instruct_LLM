...
class SelfPHPDebug:
    ...
    # Step 3: Apply debugging instructions via LLM
    def apply_debugging_instructions(self, vulnerable_code, instructions, model_url):
        prompt = f"""
        Below is a vulnerable PHP code snippet and debugging instructions:
    ...
    # Step 4: Debug unseen vulnerable code
    def debug_unseen_code(self, instruction, unseen_vulnerable_code, model_url):
        secure_code = self.apply_debugging_instructions(unseen_vulnerable_code, instruction, model_url)
        return secure_code

    # Step 5: Execute the framework
    def run(self, model_url):
        ...
        if len(self.snippets["vulnerable"]) > 1:
            ...
            for level, instruction in instructions.items():
                secure_code = self.debug_unseen_code(instruction, unseen_vulnerable, model_url)
